...
func ExtractProfile(ctx context.Context) string {
	...
	profile := os.Getenv(config.EnvConfigProfile)
	if len(profile) > 0 {
		return profile
	}
...
// GetFlagProfile makes profile from cli flags
func GetFlagProfile(ctx context.Context) *config.Profile {
	meta := extractMeta(ctx)
	profile := &config.Profile{}
	if meta.flagAPIURL != "" {
		profile.ApiUrl = &meta.flagAPIURL
	}

	if meta.flagAPIKey != "" {
		profile.ApiKey = &meta.flagAPIKey
	}
...
func GetClientProfile(ctx context.Context) (*config.Profile, error) {
	...
	flagProfile := GetFlagProfile(ctx)

	return config.MergeProfiles(profile, envProfile, flagProfile), nil
}
